...
import { Bind9Daemon, Daemon, DaemonsStats, DHCPService, DNSService, PdnsDaemon, ServicesService } from '../backend'
import {
    datetimeToLocal,
    durationToString,
    getGrafanaUrl,
...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    /**
     * Daemons statistics fetched from the server.
     */
    daemonsStats: DaemonsStats
    ...
    get noDaemons(): boolean {
        return this.daemonsStats.dhcpDaemonsTotal === 0 && this.daemonsStats.dnsDaemonsTotal === 0
    }
    ...
    ngOnInit() {
        ...
        this.subscriptions.add(
            this.serverData.getDaemonsStats().subscribe(
                (data) => {
                    this.loaded = true
                    this.daemonsStats = { ...this.daemonsStats, ...data }
                },
                (err) => {
                    this.loaded = true
