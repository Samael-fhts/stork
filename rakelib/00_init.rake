...
# Searches for prerequisites from the init file in the provided file and
# filters them to keep only the dependencies.
# Accepts a list of extra prerequisites to be added to the result.
def find_dependency_tasks(file, extra_prerequisites=[])
    if file == __FILE__
    ...
    project_dir_abs = File.expand_path('.')
    tools_dir_abs = File.expand_path('tools')
    ...
    prerequisites_tasks.each do |t|
        ...
        # Skip the in-project prerequisites that aren't targeted to the tools
        # directory. They are autogenerated files, not external dependencies.
        t_path = t.name
        if t.name.include?("/")
            # Consider only the filepath-based tasks. The executable-named
            # tasks are always external dependencies.
            t_path_abs = File.expand_path(t.name)
            if t_path_abs.start_with?(project_dir_abs) && !t_path_abs.start_with?(tools_dir_abs)
                # In-project file task that isn't in the tools directory.
                next
            end
        end
...
def find_and_prepare_deps(file, extra_prerequisites=[])
    prerequisites_tasks = find_dependency_tasks(file, extra_prerequisites)
