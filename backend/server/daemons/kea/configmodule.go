...
func (module *ConfigModule) Commit(ctx context.Context) (context.Context, error) {
	...
	for _, pu := range state.Updates {
		switch pu.Operation {
		case dbmodel.ConfigOperationKeaGlobalParametersUpdate:
			ctx, err = module.commitGlobalParametersUpdate(ctx)
		case dbmodel.ConfigOperationKeaHostAdd:
			ctx, err = module.commitHostAdd(ctx)
		case dbmodel.ConfigOperationKeaHostUpdate:
			ctx, err = module.commitHostUpdate(ctx)
		case dbmodel.ConfigOperationKeaHostDelete:
			ctx, err = module.commitHostDelete(ctx)
		case dbmodel.ConfigOperationKeaSharedNetworkAdd:
			ctx, err = module.commitSharedNetworkAdd(ctx)
		case dbmodel.ConfigOperationKeaSharedNetworkUpdate:
			ctx, err = module.commitSharedNetworkUpdate(ctx)
		case dbmodel.ConfigOperationKeaSharedNetworkDelete:
			ctx, err = module.commitSharedNetworkDelete(ctx)
		case dbmodel.ConfigOperationKeaSubnetAdd:
			ctx, err = module.commitSubnetAdd(ctx)
		case dbmodel.ConfigOperationKeaSubnetUpdate:
			ctx, err = module.commitSubnetUpdate(ctx)
		case dbmodel.ConfigOperationKeaSubnetDelete:
...
func (module *ConfigModule) ApplyHostDelete(ctx context.Context, host *dbmodel.Host) (context.Context, error) {
	...
	// Create transaction state.
	state := config.NewTransactionStateWithUpdate[ConfigRecipe](dbmodel.ConfigOperationKeaHostDelete, daemonIDs...)
	recipe := ConfigRecipe{
		Commands: commands,
		HostConfigRecipeParams: HostConfigRecipeParams{
			HostID: &host.ID,
...
func (module *ConfigModule) ApplySharedNetworkAdd(ctx context.Context, sharedNetwork *dbmodel.SharedNetwork) (context.Context, error) {
	...
	for _, lsn := range sharedNetwork.LocalSharedNetworks {
		...
		case 4:
			...
			command.Command = keactrl.NewCommandNetwork4Add(sharedNetwork4, lsn.Daemon.Name)
			commands = append(commands, command)
		...
		default:
			...
			command.Command = keactrl.NewCommandNetwork6Add(sharedNetwork6, lsn.Daemon.Name)
			commands = append(commands, command)
		}
	}
...
func (module *ConfigModule) ApplySharedNetworkUpdate(ctx context.Context, sharedNetwork *dbmodel.SharedNetwork) (context.Context, error) {
	...
	for _, lsn := range sharedNetwork.LocalSharedNetworks {
		...
		case 4:
			...
			command.Command = keactrl.NewCommandNetwork4Del(deletedSharedNetwork4, lsn.Daemon.Name)
			commands = append(commands, command)
			...
			for _, subnet := range subnets {
				for _, ls := range subnet.LocalSubnets {
					...
					command.Command = keactrl.NewCommandNetwork4SubnetAdd(sharedNetwork.Name, localSubnetID, lsn.Daemon.Name)
					commands = append(commands, command)
					break
				}
			}
		default:
			...
			command.Command = keactrl.NewCommandNetwork6Del(deletedSharedNetwork6, lsn.Daemon.Name)
			commands = append(commands, command)
			...
			for _, subnet := range subnets {
				for _, ls := range subnet.LocalSubnets {
					...
					localSubnetID := subnet.GetID(lsn.DaemonID)
					command.Command = keactrl.NewCommandNetwork6SubnetAdd(sharedNetwork.Name, localSubnetID, lsn.Daemon.Name)
					commands = append(commands, command)
					break
				}
			}
	...
	for i, exln := range existingSharedNetwork.LocalSharedNetworks {
		...
		if deletedLocalSharedNetwork != nil {
			...
			if sharedNetwork.Family == 6 {
				command.Command = keactrl.NewCommandNetwork6Del(deletedKeaSharedNetwork, deletedLocalSharedNetwork.Daemon.Name)
			} else {
				command.Command = keactrl.NewCommandNetwork4Del(deletedKeaSharedNetwork, deletedLocalSharedNetwork.Daemon.Name)
			}
...
func (module *ConfigModule) ApplySharedNetworkDelete(ctx context.Context, sharedNetwork *dbmodel.SharedNetwork) (context.Context, error) {
	...
	for _, lsn := range sharedNetwork.LocalSharedNetworks {
		...
		switch sharedNetwork.Family {
		case 4:
			command.Command = keactrl.NewCommandNetwork4Del(arguments, lsn.Daemon.Name)
		default:
			command.Command = keactrl.NewCommandNetwork6Del(arguments, lsn.Daemon.Name)
		}
		command.Daemon = lsn.Daemon
		commands = append(commands, command)
	}
	...
	daemonIDs, _ := ctx.Value(config.DaemonsContextKey).([]int64)
	// Create transaction state.
	state := config.NewTransactionStateWithUpdate[ConfigRecipe](dbmodel.ConfigOperationKeaSharedNetworkDelete, daemonIDs...)
	recipe := ConfigRecipe{
		Commands: commands,
		SharedNetworkConfigRecipeParams: SharedNetworkConfigRecipeParams{
