...
func (d *keaDaemon) getLeasefileConfigSettings(config *keaconfig.Config) (leaseDBType string, persist bool, configPath string) {
	switch d.Name {
	case daemonname.DHCPv4:
		if config != nil &&
			...
			configPath = config.DHCPv4Config.LeaseDatabase.Path
	...
	case daemonname.DHCPv6:
		if config != nil &&
			...
			configPath = config.DHCPv6Config.LeaseDatabase.Path
...
func (d *keaDaemon) ensureWatchingLeasefile(ctx context.Context, config *keaconfig.Config, maxLeaseUpdates int) error {
	leaseDBType, persist, configPath := d.getLeasefileConfigSettings(config)
