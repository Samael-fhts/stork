...
func (c *Config) unmarshalIntoAccessibleConfig(data []byte) error {
	...
	c.DHCPv4Config = nil
	c.DHCPv6Config = nil
	data, err := storkutil.NormalizeJSON(data)
	if err != nil {
		return errors.WithMessage(err, "problem normalizing Kea configuration")
	}
...
// Returns the hook library type used to alter subnets.
