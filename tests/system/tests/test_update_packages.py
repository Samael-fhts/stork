import pytest

from core.wrappers import ExternalPackages
from core import version


@pytest.mark.skip(
    reason="This test takes too much time and resources to run on every commit"
    "on CI. It should be run on demand. Now, it is disabled "
    "permanently, but it should be handled using the profile feature, "
    "similar to the tests that use the premium features."
)
def test_update_stork_from_the_latest_released_version(
    package_service: ExternalPackages,
):
    """
    Initializes the Stork server and agent with the packages from the
    CloudSmith and installs current packages.
    """
    expected_version_info = version.get_version_info()
    package_service.log_in_as_admin()
    machine = package_service.authorize_all_machines().items[0]

    state = package_service.read_machine_state(machine.id)

    agent_version = version.parse_version_info(state.agent_version)[0]
    server_version = version.parse_version_info(package_service.read_version().version)[
        0
    ]
    # We change the version in the release phase.
    # During the development the latest CloudSmith version equals to the
    # version in the GO files but during the release it is lower.
    assert agent_version <= expected_version_info
    assert server_version <= expected_version_info

    package_service.update_server_to_latest_version()
    package_service.update_agent_to_latest_version()

    state = package_service.wait_for_next_machine_states(wait_for_apps=False)[0]
    agent_version = version.parse_version_info(state.agent_version)[0]
    server_version = version.parse_version_info(package_service.read_version().version)[
        0
    ]
    assert agent_version == expected_version_info
    assert server_version == expected_version_info


@pytest.mark.skip(
    reason="This test takes too much time and resources to run on every commit"
    "on CI. It should be run on demand. Now, it is disabled "
    "permanently, but it should be handled using the profile feature, "
    "similar to the tests that use the premium features."
)
def test_update_stork_server_from_the_latest_released_version(
    package_service: ExternalPackages,
):
    """
    Initializes the Stork server and agent with the packages from the
    CloudSmith and installs current package of the server only. The agent is
    not updated. It checks if the backward compatibility is preserved.
    """
    expected_version_info = version.get_version_info()
    package_service.log_in_as_admin()
    machine = package_service.authorize_all_machines().items[0]

    state = package_service.read_machine_state(machine.id)

    agent_version_init = version.parse_version_info(state.agent_version)[0]
    server_version_init = version.parse_version_info(
        package_service.read_version().version
    )[0]
    # We change the version in the release phase.
    # During the development the latest CloudSmith version equals to the
    # version in the GO files but during the release it is lower.
    assert agent_version_init <= expected_version_info
    assert server_version_init <= expected_version_info

    package_service.update_server_to_latest_version()

    state = package_service.wait_for_next_machine_states(wait_for_apps=False)[0]
    agent_version_updated = version.parse_version_info(state.agent_version)[0]
    server_version_updated = version.parse_version_info(
        package_service.read_version().version
    )[0]
    assert agent_version_updated == agent_version_init
    assert server_version_updated == expected_version_info


@pytest.mark.skip(
    reason="This test takes too much time and resources to run on every commit"
    "on CI. It should be run on demand. Now, it is disabled "
    "permanently, but it should be handled using the profile feature, "
    "similar to the tests that use the premium features."
)
def test_update_stork_agent_from_the_latest_released_version(
    package_service: ExternalPackages,
):
    """
    Initializes the Stork server and agent with the packages from the
    CloudSmith and installs current package of the agent only. The server is
    not updated. It checks if the backward compatibility is preserved.
    """
    expected_version_info = version.get_version_info()
    package_service.log_in_as_admin()
    machine = package_service.authorize_all_machines().items[0]

    state = package_service.read_machine_state(machine.id)

    agent_version_init = version.parse_version_info(state.agent_version)[0]
    server_version_init = version.parse_version_info(
        package_service.read_version().version
    )[0]
    # We change the version in the release phase.
    # During the development the latest CloudSmith version equals to the
    # version in the GO files but during the release it is lower.
    assert agent_version_init <= expected_version_info
    assert server_version_init <= expected_version_info

    package_service.update_agent_to_latest_version()

    state = package_service.wait_for_next_machine_states(wait_for_apps=False)[0]
    agent_version_updated = version.parse_version_info(state.agent_version)[0]
    server_version_updated = version.parse_version_info(
        package_service.read_version().version
    )[0]
    assert agent_version_updated == expected_version_info
    assert server_version_updated == server_version_init
