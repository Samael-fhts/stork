...
def test_create_read_only_user_and_verify_role(page):
    ...
    assert has_users is False, "read-only user should not see 'Users' in Configuration menu"
...
def test_change_admin_password_then_revert(page):
    ...
    expect(page.get_by_text("Invalid login or password", exact=True)).to_be_visible(timeout=5000)
